function [T,Y]=...
    trBDF(Function,Tspan,Y0,Options)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
for istage=1:rkS
  ...
  for	NewtonIter=0:NewtonMaxit-1
   ...
   RHS=OdeFunction1(T+rkC(istage)*H,Z(istage,:)');
   ISTATUS(Nfun)=ISTATUS(Nfun)+1; 
   RHS=H*rkGamma*RHS;
   %disp('istage')
   %disp(istage)
   %disp('NewtonIter and Z at the top')
   %disp(NewtonIter)
   %disp(Z(istage,:))
   %disp('RHS')
   %disp(RHS)
...
%<~~ Update the solution
Y= Y + H*rkB(1)*OdeFunction1(T,Z(istage,:)');
T=T+H;
%disp('Z')
%disp(Z)
%disp('Y')
...
%/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
global ZERO ONE 
global  BEL  sdMethod rkS rkGamma rkA rkB rkC
...
rkA(1,1) = (3-sqrt(3))/6;  % rkGamma
rkA(1,2) = 0.0;					  % 0
rkA(2,1) = 1.0 - 2*(3-sqrt(3))/6; % 1-2*rkGamma 
rkA(2,2) = (3-sqrt(3))/6;  % rkGamma
...
global ZERO ONE
global TR sdMethod rkS rkGamma rkA rkB rkC
