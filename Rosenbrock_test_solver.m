function [Tspan, Y] = ...
    Rosenbrock_test_solver(Function, Tspan, Y0, Options, ICNTRL)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
for i=2:steps
    Y(:,i) = ...
        RosenbrockIntegrator(N, Y(:,i-1), T(i-1),Tspan(i), ...
            Function, Jacobian, ATOL, RTOL, ICNTRL);
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function Y = RosenbrockIntegrator(N, Y, Tstart, Tend, ...
    OdeFunction, OdeJacobian, AbsTol, RelTol, ICNTRL)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%~~~> Time loop begins below
	...
	break
        ...
        %end
        %Err = ros_ErrorNorm(N, Ytmp, Ynew, Yerr, AbsTol, RelTol, VectorTol);
        %~~~> New step size is bounded by FacMin <= Hnew/H <= FacMax
        %Fac  = min(FacMax, max(FacMin, FacSafe/(Err^(ONE/ros_ELO))));
        %Hnew = H*Fac;
        %~~~>  Check the error magnitude and adjust step size
        ISTATUS(Nstp) = ISTATUS(Nstp) + 1;
        %if  (Err <= ONE)   %~~~> Accept step
	...
	%end
	    ...
	    %~~~>   Compute the Jacobian at current time
	     ...
	     Y = Ynew;	
            ...
            %else
	     %disp(Err)
	     Fcn0 = OdeFunction(T, Ynew);
    	     Jac0 = OdeJacobian(T,Ynew);
             nj = nj + 1;
             ISTATUS(Njac) = ISTATUS(Njac) + 1;
             %if RejectMoreH
             %    Hnew = H*FacRej;
...
function Norm = l2_norm(Y1,Y2)
N = length(Y1);
Norm = 0;
for i=1:N
     Norm = Norm + (Y1(i) - Y2(i))^2;
end
Norm = sqrt(Norm/N);
Norm = max(Norm,eps);
return
