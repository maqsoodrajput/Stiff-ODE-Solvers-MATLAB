function [T, Y,T_full,Y_full, T_rej,Y_rej ,RCNTRL, ICNTRL, RSTAT, ISTAT] = ...
    Rosenbrock_test_2(Function, Tspan, Y0, Options, RCNTRL, ICNTRL)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
for i=2:steps
    [T(i), Y(i,:),T_full,Y_full,T_rej,Y_rej, IERR] = ...
        RosenbrockIntegrator(N, Y(i-1,:), T(i-1),Tspan(i), ...
            Function, Jacobian, ATOL, RTOL, RCNTRL, ICNTRL,T_full,Y_full,T_rej,Y_rej);
...
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function [ T, Y,T_full,Y_full,T_rej,Y_rej IERR ] = RosenbrockIntegrator(N, Y, Tstart, Tend, ...
    OdeFunction, OdeJacobian, AbsTol, RelTol, RCNTRL, ICNTRL,T_full,Y_full,T_rej,Y_rej)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
while ( (Direction > 0) && ((T-Tend)+Roundoff <= ZERO) || ...
    ...
    while(true)
	    ...
	    %fprintf('time step accepted: ')% ,num2str(T),' with Hnew ', num2str(Hnew), ' \n') 
            %disp(T)
	    %fprintf('with Hnew: ')
            %disp(Hnew)
	    %fprintf('Ynew accepted: ')
	    %disp(Ynew)
	if ISTATUS(Nacc) == 1
            %disp('here')
            %Y_full = Y_full';
	...
	if ISTATUS(Nacc) > 1 	
		...
		%fprintf('outside rejected')
                if isempty(T_rej)
	         %fprintf('here rejected \n')
		 T_rej = [T+Direction*H];
		 %Y_rej = [Ynew]
		 Y_rej = Ynew;
		 %disp(Y_rej)
		 ...
		 %fprintf('there rejected \n')
		 T_rej = [T_rej T+Direction*H];
		 %Y_rej = [Y_rej Ynew]
                 %disp(Ynew);
		 Y_rej = [Y_rej;Ynew];
	 	 Hnew = min(Hnew,H);
	        end
	    %fprintf('time step rejected: ')%,num2str(T),' with Hnew ', num2str(Hnew), '\n')
	    %disp(T)
	    %fprintf('with Hnew: ')
	    %disp(Hnew)
	    %fprintf('\n')
	    %fprintf('Ynew rejected: ')
	    %disp(Ynew)
            end
            RSTATUS(Nhexit) = H;
            RSTATUS(Nhnew)  = Hnew;
	...
	%fprintf('time step outside: ') %,num2str(T),'\n')
	%disp(T)
    end % UntilAccepted
