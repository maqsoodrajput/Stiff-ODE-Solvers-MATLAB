   ...
   for i = 2:length(t)
	...
	%[y1,tmp1] = newton( @(x)  x - ( un + a(1,1)*dt*f(x)),un,J);
   	%[y2,tmp2] = newton( @(x)  x - ( un - a(2,1)*dt*f(y1) +  a(2,2)*dt*f(x)),un,J);
         ...
         [Y1,isConverged]=newtonSDIRK(f_ode,yk,Y,h);
         [Y2,isConverged]=newtonSDIRK(f_ode,yk,Y,h);
...
function [Y,isConverged]=newtonSDIRK(f,fPartial,yk,Y,h)
...
%   Option for bounds and for modifying iteration parameters
  ...
  p = inputParser;
  addParameter(p,'tolfun',1e-2);  % Last argument is the default value
  addParameter(p,'tolx',1e-2);
